        ...
        for (const QString &deviceName : deviceObj.keys()) {  // Use reference here
            ...
            for (const QString &subfieldName : subfieldsObj.keys()) {  // Use reference here
                ...
                if (subfieldName == "changes"){
                    ...
                    if(!changes.checkChanges(changesFileName)){
                        qWarning() << "changes.txt could not be checked for: " << device.name << "\r";
                    }
                }
                if (subfieldName == "symbol"){
                    ...
                    if (!symbol.checkSymbol(symbolFileName, db, mainWindow)){
                        qWarning() << "symbol could not be checked for: " << device.name << "\r";
                    }
                }
                if (subfieldName == "model"){
                    QString modelFileName = localPath + subfieldsObj[subfieldName].toObject()["filename"].toString();
                    if (!model.checkModel(modelFileName, db)){
                        qWarning() << "model could not be checked for: " << device.name << "\r";
                    }
                }
                if (subfieldName == "example"){
                    QString exampleFileName = localPath + "/" + subfieldsObj[subfieldName].toObject()["filename"].toString();
                    if (!example.checkExample(exampleFileName, db)){
                        qWarning() << "example could not be checked for: " << device.name << "\r";
                    }
                }
            }
            devices.append(device);
            ...
            for (const QString &subfieldName : subfieldsObj.keys()) {  // Use reference here
                ...
                if (subfieldName == "changes"){
                    ...
                    if (!changes.processChanges(changesFileName, mainWindow)){
                        qWarning() << "changes.txt could not be processed for: " << device.name << "\r";
                    }
                }
                if (subfieldName == "symbol"){
                    ...
                    if (!symbol.processSymbol(symbolFileName, db, mainWindow)){
                        qWarning() << "symbol could not be processed for: " << device.name << "\r";
                    }
                }
                if (subfieldName == "model"){
                    ...
                    if (!model.processModel(modelFileName, db, mainWindow)){
                        qWarning() << "model could not be processed for: " << device.name << "\r";
                    }
                }
                if (subfieldName == "example"){
                    ...
                    if (!example.processExample(exampleFileName, db, mainWindow)){
                        qWarning() << "example could not be processed for: " << device.name << "\r";
                    }
                }
            }
            devices.append(device);
...
bool Utilities::backupChangeLogTxtFile(MainWindow *mainWindow) {
    ...
    if (!changesLogBckFile.exists()) {
        if (!QFile::copy(dir.changeLogPath, dir.changeLogBckPath)){
            qWarning() << "ChangeLog.txt backup file could not be copied to backup despite not being available.\r\n";
            return false;
        }
    }
    else {
        if (changesLogFile.exists()) {
            if (!QFile::remove(dir.changeLogPath)) {
                qWarning() << "ChangeLog.txt could not be removed.\r\n";
                return false;
            }
            if (!QFile::copy(dir.changeLogBckPath, dir.changeLogPath)) {
                qWarning() << "ChangeLog.txt could not be created from the backup already done.\r\n";
                return false;
            }
        }
    }
...
void Utilities::removeOldModel(QString &filename, MainWindow *mainWindow) {
    DirStruct dir;
    DirPaths dirPaths;
    dir = dirPaths.buildDirPaths(mainWindow);
    ...
    // Remove from lib
    QString fileInLibPath = dir.modelsDirPath + filename;
    QFile fileInLib = QFile(fileInLibPath);
    if (fileInLib.exists())
        fileInLib.remove();
    // Remove from Source
    QString fileInSourcePath = dir.sourceLibDirPath + filename;
    ...
    if (!removeFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
        qWarning() << "Remove.txt file: " << removePath << " could not be opened.\n";
        return;
    }
