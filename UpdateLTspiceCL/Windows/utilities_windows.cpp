...
bool Utilities_windows::unzipFile(const QString &zipFilePath, const QString &destDir) {
    ...
    if (!zip.OpenZip(zipFilePath.toStdString().c_str())) {
        qFatal() <<"Failed to open zip file: ";
        return false;
    }
    ...
    if (!entry) {
        qFatal() << "No entries found in the zip file.";
        return false;
    }
    ...
    while (entry != nullptr) {
        ...
        if (!entry->Extract(destDir.toStdString().c_str())) {
            qFatal() <<"Failed to extract file: " << QString::fromStdString(entryFileName);
            return false;
        } else {
            qDebug() << "Extracted: " << entryFileName << " to " << fullDestPath << "\r";
        }
